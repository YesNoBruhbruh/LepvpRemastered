...
public class LepvpRemasteredBaseInstance extends TeamGameInstance {
    ...
    public LepvpRemasteredBaseInstance(String displayName, String gameBase, GameMap map, int teamSize) {
        super(displayName, gameBase, map, new String[] { "Fight in a crystal pvp 1v1" }, "because you want to practice crystal pvp", true, new GameController[] { GameController.PLACE_BLOCKS, GameController.BREAK_BLOCKS, GameController.ITEM_PICKUP, GameController.BREAK_BLOCKS_MAP }, false);
    ...
    public void onEvent(Event event){
        Event event2 = event;
        if (event2 instanceof EntityDamageByEntityEvent damageByEntityEvent){
            if (!(damageByEntityEvent.getEntity() instanceof Player)){
                return;
            }
            //figure out why endercrystal is getting cancelled
            if (damageByEntityEvent.getEntity() instanceof EnderCrystal){
                Bukkit.broadcastMessage(ChatColor.GOLD + "it is the instanceof EnderCrystal");
            }

            Entity entity1 = damageByEntityEvent.getDamager();
            Entity entity2 = damageByEntityEvent.getEntity();
            Player damager;
            Player victim;

            if (entity1 instanceof Player){
                damager = (Player)entity1;
            }else{
                return;
            }
            ...
            if (victim.getHealth() - damageByEntityEvent.getFinalDamage() <= 0 && !(victim.getInventory().getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING || victim.getInventory().getItemInMainHand().getType() == Material.TOTEM_OF_UNDYING)){
                ...
                for (Team teamRaw : getTeams()) {
                    ...
                    if (!this.died.contains(GamePlayer.getPlayer(victim))) {
                        ...
                        point(GamePlayer.getPlayer(damager), team);
                        break;
                    }
                }
                getUtils().sendMessage(this.killMessages.getKillerMessage("kill", this, GamePlayer.getPlayer(victim), GamePlayer.getPlayer(damager)));
                this.died.add(GamePlayer.getPlayer(victim));
                resetPlayerLepvp(GamePlayer.getPlayer(victim));
            }
        }
        event2 = event;
        //this event handles if the player is about to die and if they are cancel and reset them.
        if (event2 instanceof EntityDamageEvent damageEvent){
            ...
            if (!this.players.contains(GamePlayer.getPlayer(victim)))
                return;
            if (victim.getHealth() - damageEvent.getFinalDamage() <= 0 && !(victim.getInventory().getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING || victim.getInventory().getItemInMainHand().getType() == Material.TOTEM_OF_UNDYING)){
        ...
        // checks if the player's team is the same one as the enemy's
        event1 = event;
